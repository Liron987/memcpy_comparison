...
// Bitwise optimized memcpy with prefetching
void memcpy_optimized_bitwise(char *dest, const char *src, size_t n) {
    size_t i;

    for (i = 0; i < n / 8; i++) {
        // Prefetch the next chunk of memory ahead of time
        if (i + 1 < n / 8) {
            __builtin_prefetch((char *)(src + (i + 1) * 8), 0, 1);  // Read, temporal
        }
...
// Casting optimized memcpy with prefetching
...
// Direct byte-by-byte copy with prefetching
void memcpy_byte_by_byte(char *dest, const char *src, size_t n) {
    size_t i;

    for (i = 0; i < n; i++) {
        // Prefetch the next byte ahead of time
        if (i + 1 < n) {
            __builtin_prefetch((char *)(src + i + 1), 0, 1);  // Read, temporal
        }
...
// Standard memcpy with prefetching
void memcpy_standard(char *dest, const char *src, size_t n) {
    // Since standard memcpy is already optimized, we can assume it's using prefetching under the hood.
    memcpy(dest, src, n);
}
...
int main() {
    ...
    // Measure time for bitwise memcpy with prefetching
    clock_t start = clock();
    memcpy_optimized_bitwise(dest1, src, SIZE);
    clock_t end = clock();
    double bitwise_time = ((double)(end - start)) / CLOCKS_PER_SEC;
    printf("Bitwise memcpy time with prefetching: %f seconds\n", bitwise_time);

    // Measure time for casting memcpy with prefetching
    start = clock();
    memcpy_optimized_casting(dest2, src, SIZE);
    end = clock();
    double casting_time = ((double)(end - start)) / CLOCKS_PER_SEC;
    printf("Casting memcpy time with prefetching: %f seconds\n", casting_time);

    // Measure time for direct byte-by-byte memcpy with prefetching
    start = clock();
    memcpy_byte_by_byte(dest3, src, SIZE);
    end = clock();
    double byte_by_byte_time = ((double)(end - start)) / CLOCKS_PER_SEC;
    printf("Byte-by-byte memcpy time with prefetching: %f seconds\n", byte_by_byte_time);
